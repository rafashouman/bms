...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("aim"):
		...
		$Mesh/template_synty_base/Root/Skeleton3D/Spine_IK.start()
		
	else:
		print('not aiming')
		$AnimationTree.set("parameters/aim_transition/transition_request", "not_aiming")
		$Mesh/template_synty_base/Root/Skeleton3D/Spine_IK.stop()
	
	var h_rot = $Mesh/template_synty_base/Root/Skeleton3D/head.global_transform.basis.get_euler().y
	...
	if Input.is_action_pressed("move_forward") || Input.is_action_pressed("move_backward") || Input.is_action_pressed("move_left") || Input.is_action_pressed("move_right"):
		...
		if $AnimationTree.get("parameters/aim_transition/current_state") == "not_aiming":
			...
			if Input.is_action_pressed("run"):
				move_speed = run_speed
				$AnimationTree.set("parameters/pistol_iwr_blend/blend_amount", lerpf($AnimationTree.get("parameters/pistol_iwr_blend/blend_amount"), 1, delta * acceleration))
			else:
				move_speed = walk_speed
				$AnimationTree.set("parameters/pistol_iwr_blend/blend_amount", lerpf($AnimationTree.get("parameters/pistol_iwr_blend/blend_amount"), 0, delta * acceleration))
	else:
		if $AnimationTree.get("parameters/aim_transition/current_state") == "not_aiming":
			$AnimationTree.set("parameters/iwr_blend/blend_amount", lerpf($AnimationTree.get("parameters/iwr_blend/blend_amount"), -1, delta * acceleration))
			move_speed = 0
		...
		if $AnimationTree.get("parameters/aim_transition/current_state") == "aiming":
			direction = $Mesh/template_synty_base/Root/Skeleton3D/head.global_transform.basis.z
